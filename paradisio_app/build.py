...
def link_with_icon(label, url, icon_key=None, cls="secondary-link"):
...
def _clean_group(groups):
    ...
    for g in groups:
        ...
        for it in items:
            ...
            replaced = False
            for (a, b), _display in ATTRIBUTE_DISPLAY_ALIASES.items():
...
def _rank_summary_items(clean_groups, category, limit):
    ...
    chosen = []
    per_group = {}
    for _score, group, label, value in candidates:
...
<a href="{pc["url"]}" class="primary-cta" target="_blank" rel="noopener" data-plausible-event="ContactClick" data-plausible-channel="{channel_type}">{ICONS.get(channel_type, "")} {pc["label"]}</a>
</div>"""
    ...
    # The sticky bar always renders, because Share is always available; the
    # Directions and Call/WhatsApp actions are added only when routable.
    parts = []
    if maps_url:
    ...
    if pc['type'] == 'WhatsApp' and wa_ok:
        ...
        if wa_url.startswith("+"):
            wa_url = "https://wa.me/" + wa_url.lstrip("+")
        parts.append(f'<a href="{wa_url}" class="sticky-call" target="_blank" rel="noopener" data-plausible-event="ContactClick" data-plausible-channel="WhatsApp">{ICONS["WhatsApp"]} WhatsApp</a>')
    parts.append(f'<button class="sticky-share" data-share-trigger>{ICONS["Share"]} Share</button>')
